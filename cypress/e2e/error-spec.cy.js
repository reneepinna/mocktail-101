describe('Network errors', () => {
  ...
  it('should throw an error', () => {
    cy.intercept('https://www.thecocktaildb.com/api/json/v1/1/filter.php?a=Non_Alcoholic', {statusCode: 404}).as('error')
    cy.visit('/')
    cy.wait('@error').its('response').should('deep.include', {statusCode: 404})

    cy.get('.error-msg').should('be.visible').should('contain', 'Error: Page not found')
    cy.get('.card').should('not.exist')
  })
